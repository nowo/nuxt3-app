...
const onSelItem = (val: any, v: any) => {
  ...
  v.active = true
  const arr = []
  state.filtering.forEach((item: any) => {
...
<template>
  <div class="filtering layout-pd">
    <el-card
      v-loading="state.tableData.loading" shadow="hover" class="filtering-list br-top-no"
    ...
    >
      <div v-for="(val, key) in state.filtering" :key="key" ref="dlRefs" class="filtering-list-flex">
        <div class="filtering-list-title">
          {{ val.title }}
        </div>
        <div class="filtering-list-item" :style="{ height: val.isMore ? 'auto' : '50px' }">
          <span
            v-for="(v, k) in val.children" :key="k" class="span" :class="v.active ? 'dd-active' : ''"
          ...
          <div v-if="val.isShowMore" class="dd-more" @click="val.isMore = !val.isMore">
            ...
            <i :class="val.isMore ? 'el-icon-arrow-down' : 'el-icon-arrow-right'" />
          </div>
        </div>
      </div>
      <div v-if="state.tableData.data.length > 0" class="flex-warp mt15 mb15">
        <el-row :gutter="15">
          <el-col
            v-for="(v, k) in state.tableData.data" :key="k" :xs="24" :sm="12" :md="8" :lg="6" :xl="4" class="mb15"
      ...
      <template v-if="state.tableData.data.length > 0">
        <el-pagination
          style="text-align: right" background :page-sizes="[10, 20, 30]"
